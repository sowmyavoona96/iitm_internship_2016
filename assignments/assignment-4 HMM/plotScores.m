clear all;
clc;
binranges = linspace(-800,-100,15);

tscores = fopen('trueScore.txt','r');
x = fscanf(tscores,'%lf');
[sizeOfX m] = size(x);
disp(sizeOfX);
...
fscores = fopen('falseScore.txt','r');
y = fscanf(fscores,'%lf');
[sizeOfY z] = size(y);
disp(sizeOfY);
...
%S = InterX([binranges;(binF)'],[binranges;(binT)']);
[x y] = intersections(binranges,binF,binranges,binT,1);
