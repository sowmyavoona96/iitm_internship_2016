...
int main(){
   // printf("%lf",findMinScore("featDir1/17za.mfcc","featDirTrain/st_ob.mfcc"));
    ...
    FILE* trainFile = fopen("lists/trainList1","r");
    ...
    double score[11][noOfTest][noOfSpeakers];
    ...
    for(i=0;i<11;i++){
        ...
        for(j=0;j<noOfTest;j++){
            for(k=noOfSpeakers*i;k<(noOfSpeakers*k+noOfSpeakers);k++){
                score[i][j][k] = findMinScore(testList[j],trainList[k]);
                mean += score[i][j][k];
            }
        }
        mean  = mean/(noOfTest*noOfSpeakers);

        for(m=0;m<noOfTest;m++){
            for(n=0;n<noOfSpeakers;n++){
                sigma += (score[i][m][n] - mean)*(score[i][m][n] - mean);
            }
        }

        sigma = sigma/((noOfTest*noOfSpeakers));
        sigma = sqrt(sigma);

        /* ----------zscore for each digit------------*/
        for(m=0;m<noOfTest;m++){
            for(n=0;n<noOfSpeakers;n++){
                score[i][m][n]= (score[i][m][n] - mean)/sigma;
            }
        }
    }
    ...
    for(i=0;i<noOfTest;i++){
        ...
        for(j=0;j<noOfSpeakers;j++){
            double scoreSp[11];
            for(m=0;m<11;m++)
                scoreSp[m] = score[m][i][j];

            int index =0;
            int ind;
            for(ind = 0;ind<11;ind++){
                if(scoreSp[index]>scoreSp[ind])
                    index = ind;
            }
        ...
        digitRec[mainIndex][i] = 1;
    }



    char  digitRecFile[] = "digitRec.txt";
